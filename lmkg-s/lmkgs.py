...
with tf.device("GPU:0"):
    ...
    def train_model(model, X, A, E, y, batch_size, epochs, matrix_mode, learning_rate = 1e-3, decay = False, use_q_loss = True, scale = 0, do_training = False, encoding_avg_time = 0):
        """
        ...
        if do_training:
            ...
            avg_time_prediction_ms = (total_pred / 30) * 1000.0
        print("The average prediction time is " + str(avg_time_prediction_ms))
        print("The average encoding time is " + str(avg_time_encoding_ms))
        ...
        correct_estimate = check_network_estimates(preds, y, statistics_file_name = statistics_file_name, print_time=print_time, nb_outliers = nb_outliers)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--query-join', type=int, default=2, help='The size of the join.')
    ...
    if "star" in query_type and 'all' not in query_type:
        ...
        X, A, E, y, encoding_time = complex_reader.read_combined(d, b, n, e, file_name_star, file_name_chain, train_tuples, test_mode)
    elif 'allstar' in query_type or 'allchain' in query_type or 'alltypessizes' in query_type:
        X, A, E, y, encoding_time = complex_reader.read_combined_all_sizes_star_or_chain(d, b, n, e, all_file_names, train_tuples)
    elif 'complex' in query_type:
        if not do_training:
    ...
    '''train the model'''
    train_model(nn_model, X, A, E, y, batch_size, epochs, decay = decay, matrix_mode = matrix_mode, scale = scale, do_training=do_training, encoding_avg_time=avg_time_encoding_ms)
